...
sys.path.append(os.path.join('4_Python_Data_Visualization', 'Week_3', 'practice_project'))
import practice_project as week3_pp
...
def draw_cancer_risk_map(joined_csv_name, map_name, num_counties = None):
    """
    Given names of joined CSV file and PNG map of USA, draw cancer-risk data for 
    counties overlaid on USA map.
    ...
    joined_cancer_risk_table = week3_pp.read_csv_file(joined_csv_name)
...
joined_csv = week3_pp.cancer_joined
usa_map = os.path.join('4_Python_Data_Visualization',
                       'Week_1','practice_project',
                       'USA_Counties_1000x634.png')
                       
draw_cancer_risk_map(joined_csv, usa_map, 200)
