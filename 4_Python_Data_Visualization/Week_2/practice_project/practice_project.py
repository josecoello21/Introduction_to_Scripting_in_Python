...
svg_file = os.path.join('4_Python_Data_Visualization',
                        'Week_2','practice_project',
                        'USA_Counties_2014.svg')
...
def get_county_attributes(svg_file_name):
    ...
    """
    file = minidom.parse(svg_file_name)
    tag_path = file.getElementsByTagName('path')
    
    id_d_attribute = [(path.getAttribute('id'), path.getAttribute('d')) for path in tag_path]
    
    return id_d_attribute
...
def get_boundary_coordinates(boundary_data):
    ...
    """
    my_str = ''
    for string in boundary_data:
        if string not in ['M', 'L', 'z']:
            my_str += string
    
    return [tuple(map(lambda num: float(num), lst.split(','))) for lst in my_str.split()]
...
def process_county_attributes(svg_file_name, csv_file_name):
    ...
    my_att = get_county_attributes(svg_file_name)
    
    extract = lambda tpl, idx: tpl[idx]
    
    my_str = list(map(lambda x: extract(tpl=x, idx=1), my_att))
    
    my_id = list(map(lambda x: extract(tpl=x, idx=0), my_att))
    
    coord = list(map(get_boundary_coordinates, my_str))
    
    centers = list(map(compute_county_center, coord))
    
    my_data = {}
    
    for idx in range(len(centers)):
        row = zip(['FIPS-code', 'x-coord', 'y-coord'], [my_id[idx]] + centers[idx])
        my_data[my_id[idx]] = dict(row)
    
    # Ouput CSV file
    with open(csv_file_name, 'w', newline = '') as csvfile:
        fieldnames = ['FIPS-code', 'x-coord', 'y-coord']
        writer = csv.DictWriter(csvfile, fieldnames = fieldnames)
        
        writer.writeheader()
        for key in my_data:
            writer.writerow(my_data[key])


# Output CSV file should have 3143 rows
    
csv_file = os.path.join('4_Python_Data_Visualization',
                        'Week_2','practice_project',
                        'USA_Counties_with_FIPS_and_centers.csv')
                        
process_county_attributes(svg_file, csv_file)
